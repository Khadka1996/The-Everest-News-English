...
const International = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        // Backend returns up to 6 items sorted by createdAt descending
        const response = await axios.get(`${API_URL}/api/english/category/international?limit=6&sortBy=createdAt&sortOrder=desc`);
  ...
  return (
    <div className="mx-3 md:mx-10 lg:mx-18 py-6">
      <div className="mb-6 mt-2">
        <AdvertisementComponent position="english_international" />
      </div>
      ...
      {articles.length > 0 && (
        <div
          onClick={() => window.location.href = 'category/international'}
