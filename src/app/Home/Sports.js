...
const Card = React.memo(({ id, headline, imageUrl, createdAt, category, onClick, onCategoryClick }) => {
...
const Sports = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_URL}/api/english/category/sports`, {
  ...
  return (
    <div className="mx-3 md:mx-10 lg:mx-18 py-6">
      <div className="mb-6 mt-2">
        <AdvertisementComponent position="english_sports" />
      </div>
      ...
      {articles.length > 0 && (
        <div
          onClick={() => window.location.href = 'category/sports'}
